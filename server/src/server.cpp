        ...
        while (ctx->running.load()) {
            ...
            try {
                ...
                for (const auto& net : current_interfaces) {
                    if (net.usingNow()) {
                        activeIface = net.ifName();
                        break;
                    }
                }
                if (!activeIface.empty() && activeIface != lastBoundIface) {
                    LOG_INFO(LogModule::WEAK_MGR, "traffic analysis: rebinding to active interface " << activeIface);
                    ctx->weak_mgr->stopTrafficAnalysis();
                    ctx->weak_mgr->startTrafficAnalysis(activeIface, 10);
                    lastBoundIface = activeIface;
                }
